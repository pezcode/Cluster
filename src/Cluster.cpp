...
void Cluster::UI::initialize()
{
	...
	bgfx::TextureHandle tex = bgfx::createTexture2D((uint16_t)tex_w, (uint16_t)tex_h, false, 1,
		bgfx::TextureFormat::RGBA8, 0, bgfx::copy(tex_data, tex_w * tex_h * bytes));
...
void Cluster::UI::update(float time)
{
	...
	if(config.showPerfOverlay)
    {
		...
		if(config.overlays.fps)
		{
			ImGui::Separator();
            ImGui::Text("FPS");
            ImGui::PlotLines("", fpsValues, IM_ARRAYSIZE(fpsValues), offset + 1, nullptr, 0.0f, 200.0f, ImVec2(150, 50));
        ...
        if (config.overlays.frameTime)
        {
            ...
            ImGui::PlotLines("", frameTimeValues, IM_ARRAYSIZE(frameTimeValues), offset + 1, nullptr, 0.0f, 30.0f, ImVec2(150, 50));
        ...
        if (config.overlays.gpuMemory)
        {
            ...
            ImGui::PlotLines("", gpuMemoryValues, IM_ARRAYSIZE(gpuMemoryValues), offset + 1, nullptr, 0.0f, max, ImVec2(150, 50));
...
void Cluster::UI::imageTooltip(ImTextureID tex, ImVec2 tex_size, float region_size)
{
    ...
    if(ImGui::IsItemHovered())
    {
        ...
        ImGui::Image(tex, ImVec2(region_size * zoom, region_size * zoom), uv0, uv1, ImVec4(1.0f, 1.0f, 1.0f, 1.0f),
